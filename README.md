...
The lowest level of our object oriented architecture is the SCP class. The goals of these classes are to provide the ability for our computers to speak these different languages. These classes are usually referred to as "serial communication drivers". (However, in our codebase, they are just called "drivers")

Once we have a class that lets us utilize an SCP, we need to write code to decide what to actually say to the device. This will be the device class. Since each device uses an SCP to communicate, we can create a class that inherits properties from the respective SCP class. This will allow us to use the methods provided by the SCP class so we can speak the proper language. This tutorial will focus on developing the device class for the MCP23017.
...
I know that was a lot of information, but now you should understand the mechanisms we are going to use to interact with the pins on our MCP23017. This section of the tutorial will involve implementing functions to configure, monitor, and control the pins on an MCP23017. 
 ...
 Let's start by looking at the I2C serial communication parent class. Take a look at the definition of the I2c class in [drivers/include/i2c.h](drivers/include/i2c.h). Since the Mcp23017 class is a child of the I2c class, it's important to unerstand the protected methods we will have at our disposal. These methods are already implemented, but we will be using them to actually send messages on the I2C bus. The two important methods we will use in this tutorial are the `uint8_t read_from_reg(uint8_t reg)` method and the `int write_data(uint8_t reg, T val)`. Feel free to look at the implemtations of these methods in [drivers/src/i2c.cpp](drivers/src/i2c.cpp), but, for this tutorial, reading the header comments for these methods should be sufficient.
 ...
 **Note**: The data in an I2C message is always one or more bytes long. This means that we can't just tell the device to write or read a single bit of a register. We will always be dealing with all 8 bits, AKA a byte, at once.
 ...
 Now that we know what functions we will use to read and write the MCP23017 registers, we can start implementing the functions defined in [devices/include/mcp23017.h](devices/include/mcp23017.h). The stubs are layed out for you in [devices/src/mcp23017.cpp](devices/src/mcp23017.cpp). If you feel confident in implementing these functions, give it a try! Feel free to skip to [Testing](#testing) section once you are done. If you still feel like you need some help, I will go over each of these methods one by one.
 ...
 **The Constructor**<br>
 Even though it looks empty, this function is actually already implemented. The MCP23017 is a relatively simple device, and we don't really need any extra fields to hang onto. So, all that's happening here is we're calling the constructor of the parent I2c class. We will need to take in the device's bus number and I2C address, though, as these are required by the parent class constructor.

 **`get_dir`**<br>
 This function should return the value of a specific pin's direction. To do this, we will need to do a few things. First, we will need to read from the iodir register. Second, since we have to read the value of the entire register (remember we can't just read one bit), we have to do some bitwise arithmetic to figure out what part of the byte we read corresponds to the pin we are looking for. Finally, we can return that value.
